...
void create_rand_doors(void) {
	...
	//this is a stupid way to create a endpoint ,use a bug that was fix patched it (see door_teleport and draw_door)
	Doors[0].in_door.x_pos = Doors[0].out_door.x_pos;Doors[0].in_door.y_pos = Doors[0].out_door.y_pos;
